...
| いいね数 | 累計いいね数 |
| コメント数 | 件数のみ（中身は取得しない） |
| ジャケット画像URL | `image_url`（ダッシュボードで表示） |
| 作成日時 | `created_at`（新曲判定に使用） |

- 新曲が追加された場合は自動で検出・追加する（毎回アーティストページから曲一覧を取得する設計）

### 2.3 フォロワーデータ
SUNO Profile APIの `stats` フィールドからフォロワー数を取得する。

| データ項目 | 説明 |
|-----------|------|
| フォロワー数 | `stats.followers_count` |
| フォロー数 | `stats.following_count` |

- 3時間ごとに `data/followers.csv` に追記し、推移を記録

### 2.2 トレンドデータ
SUNO Discover API（`POST studio-api-prod.suno.com/api/discover/`）で各曲のランクイン状況を確認する。
...
| 保存場所 | data/ ディレクトリ配下 |
| ファイル構成 | `{artist}.csv`（アーティスト別）+ `trends.csv`（トレンド共通）+ `followers.csv`（フォロワー推移）+ `artists.json`（一覧） |
| Git管理 | data/ はリポジトリにコミット（GitHub Actionsが自動push） |

### 3.3 ダッシュボード
| 項目 | 内容 |
|------|------|
| 形式 | HTML + JavaScript（単一ページ） |
| グラフライブラリ | Chart.js + chartjs-adapter-date-fns + chartjs-plugin-annotation |
| デザイン | ライトテーマ（Applify風）、Interフォント、白カード+影 |
| ホスティング | GitHub Pages |
...
| セクション | 内容 |
|-----------|------|
| 統計カード | 総再生数、総いいね、楽曲数、フォロワー数、トレンドランクイン |
| 総合チャート | 横棒グラフ（再生数/いいね数ランキング）。上位10曲を色分け、新曲は赤+🆕バッジ |
| フォロワー推移 | 折れ線チャート（アーティスト別） |
| 曲別詳細 | セレクタで曲を選択→ジャケット画像、再生数/いいね/いいね率/コメント/トレンド状態のカード、再生数・いいね推移チャート |
| データテーブル | 楽曲一覧（ソート可能: 曲名/アーティスト/再生数/いいね/いいね率/コメント/変化）、トレンドランクイン履歴 |

#### 新曲ハイライト
- 作成日から1ヶ月以内の曲を「新曲」として赤文字+NEWバッジで表示
- 総合チャート、テーブル、曲セレクタすべてに反映

#### アーティストフィルタ
- アーティスト選択時にアバター画像を表示
- 曲セレクタ・テーブルも選択アーティストでフィルタ
...
- SUNO Discover API によるトレンド検出
- アーティスト別CSV データ保存
- フォロワー数の計測・推移記録（followers.csv）
- ジャケット画像・作成日時の取得
